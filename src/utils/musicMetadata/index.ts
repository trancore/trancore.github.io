...
import {
  ...
  getStringUTF8,
  getStringUTF16,
  transformImageInBase64,
  transformUTF8ToUTF16,
} from "~/utils/characterSet";
...
function ID3v2TagReader(musicData: Uint8Array<ArrayBuffer>) {
  ...
  const ID3Frame = {
     ...
     */
    isID3FrameID: (IDName: keyof typeof ID3_FRAME_ID, index: number): boolean =>
     ...
     */
    readText: (index: number, size: number): { text: string; skip: number } => {
     ...
     */
    readID3FrameSize: (index: number): number =>
      // 24ビット左にシフト
     ...
     */
    isFrameHeaderFmtFlgIncludeOrgSize: (index: number): boolean =>
     ...
     * @returns {number} フレームサイズ
     */
    readFrameBodySizeV24ForAPIC: (): number =>
      // 21ビット左にシフト
      (musicData[4] << 21) |
      // 14ビット左にシフト
      (musicData[5] << 14) |
     ...
     */
    readMimeType: (beginIndex: number): string => {
...
function getMetadataMp3(musicData: Uint8Array<ArrayBuffer>): {
  ...
  if (isID3v2()) {
    ...
    if (mimeType !== "") {
      musicMetadata.albumWork = transformImageInBase64(mimeType, binary);
    }
...
function vorbisCommentTagReader(musicData: Uint8Array<ArrayBuffer>) {
  ...
  function isFLAC(): boolean {
    // ASCII表記でfLaC
    return getIntNumberFromBinary(musicData, 0, 4) === 0x664c6143;
  }
    ...
    for (;;) {
      ...
      switch (
        ...
        case 6: {
          ...
          // 説明
          // biome-ignore lint/correctness/noUnusedVariables: 一旦無視
          let description = "";
...
function RIFFTagReader(musicData: Uint8Array) {
  ...
  function readRIFFs(): void {
    ...
    for (;;) {
      ...
      // @ts-expect-error: 実際には使わないため無視
      // biome-ignore lint/correctness/noUnusedVariables: 実際には使わないため無視
